...
class DINOv2_ResNet34_Hybrid(nn.Module):
    def __init__(self, clinical_feature_size=13, num_classes=2, dropout_rate=0.4):
        super().__init__()

        self.backbone = torch.hub.load(
            'facebookresearch/dinov2', 'dinov2_vits14'
        )
        self.embed_dim = 384  # DINOv2 ViT-S/14 维度
    ...
    def forward_image_branch(self, x):
        ...
        feat_map_upsampled = F.interpolate(feat_map, size=(56, 56), mode='bilinear', align_corners=False)
...
def prepare_data(csv_path):
    ...
    categorical_cols = ['Calcification', 'Margin', 'Echo', 'Internal Structure', 'Aspect ratio']
    confidence_cols = ['conf_calcification', 'conf_margin', 'conf_echo', 'conf_internal', 'conf_Aspect ratio']
    ...
    train_df = data[data['dataset'] == 1].copy()
    valid_df = data[data['dataset'] == 2].copy()
    test_df = data[data['dataset'] == 3].copy()
...
if __name__ == "__main__":
    ...
    csv_file = r"D:\文章--甲状腺基因\文本＋图像\用两张图片进行训练_划分后.csv"
